function [out1,out2,out3] = ginputzp(arg1)
%GINPUT Graphical input from mouse with online zooming and panning ability.

%   [X,Y] = GINPUTZS(N) gets N points from the current axes and returns
...
%
%   [X,Y] = GINPUTZP gathers an unlimited number of points until the
%   return key is pressed.
%
%   [X,Y,BUTTON] = GINPUTZP(N) returns a third result, BUTTON, that
...
%       [x,y] = ginputzp(5);
%
%       [x, y, button] = ginputzp(1);
%
%   See also GTEXT, WAITFORBUTTONPRESS.

% How to use it:
% The left mouse button cannot be used for data selection anymore.
% Instead, you can use it for zooming and panning while you are making
% selection with middle or right mouse buttons.
% To zoom in, after running the ginputzp function and red cross first press
% "z" button on keyboard and then use left mouse or
% mouse wheel to zoom in and also zoom out, and to reset zoom use double left
% click. To pan, while ginputzp is running, press x on the keyboard and use
% left click and drag to pan and do the selection with middle or right mouse button.
% press enter to finish point selection.
